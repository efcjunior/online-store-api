...
export const create = (order: BasicOrder, callback: Function) => {
    ...
    db.query(queryString,[order.product.id, order.customer.id, order.productQuantity], 
        (err, result) => {
            if(err) {callback(err)};
...
export const update = (order: Order, callback: Function) => {
    ...
    db.query(queryString, [order.product.id, order.productQuantity, order.orderId], (err, result) => {
